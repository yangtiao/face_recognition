    ...
    for dir_item in os.listdir(data_path):
        ...
        if os.path.isdir(full_path):    
            ...
            if dir_item.endswith('.jpg') or dir_item.endswith('.bmp'):
                ...
                cv2.imwrite(os.path.join('./resize/meeting', data_path.split('\\')[-1], dir_item), image)
...
def load_dataset(data_path):
    ...
    newlabels = []
    for label in labels:
        if label.endswith('glassesOne'):
            newlabels.append(0)
        elif label.endswith('glassesTwo'):
            newlabels.append(1)
        elif label.endswith('jessica'):
            newlabels.append(2)
        elif label.endswith('linda'):
            newlabels.append(3)
        elif label.endswith('ZBnan'):
            newlabels.append(4)
        else:
            newlabels.append(5)
